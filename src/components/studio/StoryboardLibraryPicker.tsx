...
export function StoryboardLibraryPicker({ open, onOpenChange, projectId, subprojectId, maxSelect, onConfirm, onSetFirstFrame }: Props) {
  ...
  const [scope, setScope] = useState<'all' | 'global' | 'project' | 'subproject'>(subprojectId ? 'subproject' : projectId ? 'project' : 'all');
  ...
  const visible = rows.filter(r => {
    ...
    if (scope === 'subproject') {
      ...
      if (r.subproject_id !== subprojectId) return false;
    }
      ...
      <DialogContent className="bg-zinc-950 border-white/10 max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Clapperboard className="w-4 h-4 text-[#00ff88]" /> Insert From Storyboards
          </DialogTitle>
          <DialogDescription>Pick saved storyboard frames to use as reference images.</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={scope} onValueChange={(v) => setScope(v as typeof scope)}>
            <SelectTrigger className="w-[200px] bg-background/50 h-9"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="rounded-xl border border-white/10 bg-card/30 p-3 min-h-[280px] max-h-[420px] overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-16 text-sm text-muted-foreground">
              No storyboards found. Add some in the Storyboard tab.
            </div>
          ) : (
                  ...
                  <button key={r.id} type="button" onClick={() => toggle(r.id)}
                    ...
                    {isPicked && (
                      <div className="absolute inset-0 bg-[#00ff88]/20 flex items-center justify-center">
                        <div className="bg-[#00ff88] text-black rounded-full p-1.5"><Check className="w-4 h-4" /></div>
                      </div>
                    )}
