...
import { Loader2, Folder, Check, Layers } from 'lucide-react';
...
export function AssetLibraryPicker({ open, onOpenChange, projectId, subprojectId, maxSelect, onConfirm }: Props) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [rows, setRows] = useState<AssetRow[]>([]);
  const [scope, setScope] = useState<'all' | 'project' | 'subproject'>(subprojectId ? 'subproject' : projectId ? 'project' : 'all');
  ...
  useEffect(() => {
    if (!open) return;
    setPicked(new Set());
    setScope(subprojectId ? 'subproject' : projectId ? 'project' : 'all');
    (async () => {
      setLoading(true);
  ...
  }, [open, toast, projectId, subprojectId]);

  const visible = rows.filter(r => {
    if (scope === 'project' && projectId && r.project_id !== projectId) return false;
    if (scope === 'subproject') {
      if (projectId && r.project_id !== projectId) return false;
      if (subprojectId && r.subproject_id !== subprojectId) return false;
    }
      ...
      <DialogContent className="bg-zinc-950 border-white/10 max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Layers className="w-4 h-4 text-[#00ff88]" /> Insert From Assets
          </DialogTitle>
          <DialogDescription>Pick saved assets to use as reference images.</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={scope} onValueChange={(v) => setScope(v as typeof scope)}>
            <SelectTrigger className="w-[200px] bg-background/50 h-9">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All assets</SelectItem>
              <SelectItem value="project" disabled={!projectId}>This project</SelectItem>
              <SelectItem value="subproject" disabled={!subprojectId}>This subproject</SelectItem>
            </SelectContent>
          </Select>
        </div>
