...
export interface CortexTerminalProps {
  /** Module identifier sent to the edge function, e.g. "meetings", "calendar", "availability" */
  module: string;
  /** Human-readable label shown in the header */
  label: string;
  /** Optional subtitle / context hint */
  hint?: string;
  /** Placeholder text for the input */
  placeholder?: string;
  /** Edge function name to invoke (default: "clawd-bot") */
  edgeFunction?: string;
  /** Optional sub-module for AI gen tabs */
  subModule?: string;
}
...
export default function CortexTerminal({
  ...
  const [lines, setLines] = useState<TerminalLine[]>([
    { id: '0', type: 'info', text: `⚡ ${label} terminal ready. Prompt-driven control for ${module}.`, timestamp: new Date() },
  ]);
    ...
    try {
      const { data, error } = await supabase.functions.invoke(edgeFunction, {
        body: {
          prompt: cmd,
          module,
          ...(subModule ? { sub_module: subModule } : {}),
        },
      });
      ...
      if (data?.type === 'clarify') {
        ...
        // Handle standard clawd-bot JSON responses
        addLine('success', `✓ ${typeof data.data === 'string' ? data.data : JSON.stringify(data.data, null, 2)}`);
      } else if (data?.error) {
  ...
  }, [input, loading, module, subModule, edgeFunction, addLine]);
  ...
  return (
    <div className={cn(
      'border-t border-border/50 bg-[hsl(var(--card))] transition-all duration-300 mt-6 rounded-lg',
      ...
      >
        ...
        <span className="opacity-60 truncate">{hint || `prompt-driven · ${module}`}</span>
      ...
      {expanded && (
        <div className="flex flex-col h-[calc(100%-3rem)]">
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-2 font-mono text-xs space-y-0.5 max-h-60">
            ...
            {loading && <div className="text-muted-foreground/50 animate-pulse">processing…</div>}
          </div>
...
function formatResult(data: any): string {
  ...
  if (data.room_code) parts.push(`room: ${data.room_code}`);
  if (data.message) parts.push(data.message);
  if (Array.isArray(data)) parts.push(`${data.length} results`);
