...
export function XFeedPanel() {
    ...
    <ScrollArea className="h-full">
      <div className="divide-y divide-border pb-8">
        {list.map((tw) => (
          <article
            key={tw.id}
            className="p-3 hover:bg-muted/30 transition-colors group cursor-pointer relative"
            ...
            <div className="flex gap-2.5">
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex items-center gap-1 min-w-0">
                  <span className="text-sm font-bold text-foreground truncate">{cleanTweetText(tw.display_name || tw.user)}</span>
                  {tw.verified && (
  ...
  return (
    <div className="glass-card rounded-xl border border-border overflow-hidden flex flex-col" style={{ height: 'calc(380vh - 420px)' }}>
      {/* Header */}
