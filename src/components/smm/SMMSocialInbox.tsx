...
interface Props { platform: SocialPlatform }

export default function SMMSocialInbox({ platform }: Props) {
  ...
  const apiGetConversations = (u: string) =>
    isTikTok ? smmApi.getTikTokConversations(u) : smmApi.getIGConversations(u);
  const apiGetMedia = (u: string) =>
    isTikTok ? smmApi.getTikTokMedia(u) : smmApi.getIGMedia(u);
  const apiGetComments = (u: string, id: string) =>
    isTikTok ? smmApi.getTikTokComments(u, id) : smmApi.getIGComments(u, id);
  const apiSendDM = (u: string, r: string, m: string) =>
    isTikTok ? smmApi.sendTikTokDM(u, r, m) : smmApi.sendIGDM(u, r, m);
  const apiReplyComment = (u: string, c: string, m: string) =>
    isTikTok ? smmApi.replyToTikTokComment(u, c, m) : smmApi.replyToIGComment(u, c, m);

  useEffect(() => {
    if (!username) return;
    apiGetConversations(username).then(setConversations);
    apiGetMedia(username).then(setMedia);
    setActiveConv(null);
  ...
  useEffect(() => {
    if (selectedMedia && username) apiGetComments(username, selectedMedia).then(setComments);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedMedia, username, platform]);
  ...
  const refreshConversations = async () => {
    if (!username) return;
    const convs = await apiGetConversations(username);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await apiSendDM(username, activeConversation.participant_id, newMsg.trim());
  ...
  const handlePrivateReply = async (commentId: string) => {
    if (!username) return;
    try {
      await apiReplyComment(username, commentId, 'Thanks for your comment! 🙏');
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        <Button variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={() => navigateToTab('analytics', { platform })}>
          <BarChart3 className="h-3 w-3" /> Profile Analytics
        </Button>
        <Button variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={() => navigateToTab('history', { platform })}>
          <History className="h-3 w-3" /> Last 10 Posts
        </Button>
        <Button variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={() => navigateToTab('composer', { platform })}>
          <PenLine className="h-3 w-3" /> Create {platformLabel} Post
        </Button>
      </div>
