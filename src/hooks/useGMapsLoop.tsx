...
interface GMapsLoopContextType {
  loopState: GMapsLoopState;
...
const GMapsLoopContext = createContext<GMapsLoopContextType | null>(null);

const now = () => new Date().toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });

export function GMapsLoopProvider({ children }: { children: React.ReactNode }) {
  const [loopState, setLoopState] = useState<GMapsLoopState>(() => {
    try {
      ...
      if (saved) {
        ...
        return {
          ...
          progressLog: parsed.active ? [{ step: -1, label: '♻️ Resumed', status: 'done', detail: 'GMaps loop restored from previous session', ts: now() }] : [],
        };
      }
    } catch {}
  ...
  const addLog = useCallback((entry: GMapsLoopState['progressLog'][0]) => {
  ...
  const runGenerate = useCallback(async () => {
    ...
    setLoopState(prev => ({
      ...
      progressLog: [
        ...
        { step: -1, label: '📍 GMaps Agent', status: 'done', detail: `Targeting: "${searchTerm}" in ${location}`, ts: now() },
      ],
    }));
  ...
  return (
    <GMapsLoopContext.Provider value={{
      loopState,
      ...
      {children}
    </GMapsLoopContext.Provider>
  );
}

export function useGMapsLoop() {
  const ctx = useContext(GMapsLoopContext);
  if (!ctx) throw new Error('useGMapsLoop must be inside GMapsLoopProvider');
  return ctx;
}
