...
import { Mail, CheckCircle, ArrowLeft, Globe, Timer } from 'lucide-react';
import { motion } from 'framer-motion';
import { useMetaPixel } from '@/hooks/useMetaPixel';

export default function ThankYouWebDesign() {
  const navigate = useNavigate();
  useMetaPixel('3210977432422611', true);
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground">
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 md:px-12 py-4 md:py-5 relative z-30">
        <button onClick={() => navigate('/webdesign')} className="flex items-center gap-2 group">
      ...
      <main className="relative z-10 flex flex-col items-center px-4 sm:px-6 pt-8 sm:pt-16 pb-20">
        <motion.div className="max-w-lg w-full text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <p className="text-[9px] sm:text-[10px] md:text-xs tracking-[0.3em] sm:tracking-[0.4em] uppercase text-muted-foreground/60 mb-3">Web Design</p>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold tracking-tight mb-2">Thank You for Your Interest!</h1>
          <p className="text-xs sm:text-sm text-muted-foreground/60 font-light mb-10 sm:mb-14">We're excited to build your new website. Here's what's next.</p>
        </motion.div>

        <div className="max-w-md w-full flex flex-col gap-4 sm:gap-5">
          {[
            { icon: Mail, n: 1, title: 'Check Your Email', desc: "We've sent a confirmation with your project details." },
            { icon: Globe, n: 2, title: 'Design Kickoff', desc: 'Our team will reach out within 24 hours to start your website build.' },
            { icon: CheckCircle, n: 3, title: "You're All Set", desc: 'Your custom website will be live before you know it.' },
