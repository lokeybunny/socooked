...
type ApiKey = (typeof validApis)[number]
...
Deno.serve(async (req) => {
  ...
  let selectedApi: ApiKey | null = null
  if (req.method === 'POST') {
    try {
      const body = await req.json()
      const incomingApi = typeof body?.api === 'string' ? body.api.toLowerCase() : null
      if (incomingApi) {
        if (!validApis.includes(incomingApi as ApiKey)) {
          return new Response(JSON.stringify({ success: false, error: `Invalid api key. Use one of: ${validApis.join(', ')}` }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          })
        }
        selectedApi = incomingApi as ApiKey
      }
    } catch {
      // allow empty body for "check all"
    }
  }

  const shouldRun = (api: ApiKey) => !selectedApi || selectedApi === api
  const results: CreditInfo[] = []

  // ─── OpenRouter ───────────────────────────────────────
  if (shouldRun('openrouter')) {
    try {
      ...
      results.push({ name: 'OpenRouter', balance: null, unit: 'USD', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── Higgsfield ──────────────────────────────────────
  if (shouldRun('higgsfield')) {
    try {
      ...
      results.push({ name: 'Higgsfield AI', balance: null, unit: 'credits', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── Deepgram ────────────────────────────────────────
  if (shouldRun('deepgram')) {
    try {
      ...
      results.push({ name: 'Deepgram', balance: null, unit: 'USD', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }
  ...
  if (shouldRun('v0')) {
    try {
      ...
      results.push({ name: 'V0 Designer', balance: null, unit: 'credits', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── Lovable AI (Nano Banana) ────────────────────────
  if (shouldRun('lovable')) {
    try {
      ...
      results.push({ name: 'Lovable AI (Nano Banana)', balance: null, unit: 'Cloud credits', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── Telegram Bot ────────────────────────────────────
  if (shouldRun('telegram')) {
    try {
      ...
      results.push({ name: 'Telegram Bot', balance: null, unit: 'free', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── RingCentral ─────────────────────────────────────
  if (shouldRun('ringcentral')) {
    try {
      ...
      results.push({ name: 'RingCentral', balance: null, unit: 'subscription', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  // ─── Instagram ───────────────────────────────────────
  if (shouldRun('instagram')) {
    try {
      ...
      results.push({ name: 'Instagram API', balance: null, unit: 'free', status: 'error', details: e instanceof Error ? e.message : 'Unknown error' })
    }
  }

  return new Response(JSON.stringify({ success: true, data: results, selected_api: selectedApi }), {
