...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const embed = {
      ...
      fields: [
        ...
        },
        {
          ...
          value: [
            `**${tweetsToday}** tweets posted today — if every tweet got a click:`,
            `📌 Potential: **${tweetsToday}** × $0.05 = **$${(tweetsToday * SHILLER_RATE).toFixed(2)}**`,
            `✅ Claimed: **$${dailyEarnings.toFixed(2)}** (${tweetsToday > 0 ? ((clicksToday / tweetsToday) * 100).toFixed(0) : 0}% capture rate)`,
            `❌ Left on table: **$${((tweetsToday * SHILLER_RATE) - dailyEarnings).toFixed(2)}**`,
            ``,
            `🤑 Every verified click = **$0.05** — don't leave money on the table!`,
          ].join("\n"),
          inline: false,
        },
      ],
    ...
    const tgMessage = [
      intro,
      "",
      `📊 *This Hour:* ${tweetsThisHour} tweets • ${clicksThisHour} verified clicks`,
      `📈 *Today:* ${tweetsToday} tweets (${hourlyAvg}/hr) • ${totalClicksToday} clicks (${clicksToday} verified)`,
      "",
      `👥 *Active Team:*`,
