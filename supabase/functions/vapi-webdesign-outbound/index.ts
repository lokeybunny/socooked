...
serve(async (req) => {
  ...
  try {
    ...
    const { action, customer_id, phone, full_name, business_name, message } = await req.json();
    ...
    if (action === "trigger_call") {
      ...
      const customerNumber = (() => {
        ...
        if (ph.length !== 10) {
          console.error(`[vapi-webdesign] Invalid phone: "${ph}" (original: "${phone}")`);
          return null;
        }
        return `+1${ph}`;
      })();
        ...
        body: JSON.stringify({
          ...
          assistantOverrides: {
            variableValues: {
              ...
              leadName: full_name,
              firstName,
              businessName: business_name || "their business",
              message: message || "",
            },
      ...
      // Save vapi call info to customers table by phone lookup
      ...
      if (custRow) {
        await sb.from("customers").update({
          meta: {
            ...
            vapi_assistant: "webdesign",
            vapi_triggered_at: new Date().toISOString(),
          },
        }).eq("id", custRow.id);
        console.log(`[vapi-webdesign] Saved call_id to customer ${custRow.id}`);
      } else if (customer_id) {
        await sb.from("customers").update({
          meta: {
            vapi_call_id: vapiData.id,
      ...
      console.log(`[vapi-webdesign] Call triggered for ${firstName} (${customerNumber}), call_id: ${vapiData.id}`);
    ...
    console.error("vapi-webdesign-outbound error:", err);
