...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "subscribers") {
      // ManyChat doesn't have a "list all subscribers" GET endpoint easily.
      // Instead we load conversations from our communications table.
      const sb = getSupabaseAdmin();
      ...
      // Group by subscriber (external_id or from_address)
    ...
    if (action === "messages") {
      ...
      const { data: msgs, error } = await sb
        ...
        .eq("type", "instagram")
        .eq("provider", "manychat")
        .or(`external_id.eq.${subscriberId},and(to_address.eq.${subscriberId})`)
        .order("created_at", { ascending: false })
        .limit(50);
    ...
    return new Response(
      JSON.stringify({ error: "Unknown action. Use ?action=subscribers|messages|send|webhook|find|subscriber" }),
