  ...
  if (url.searchParams.get("action") === "test-latest-axiom") {
    try {
      ...
      for (const cid of channelIds) {
        const res = await fetch(`${DISCORD_API}/channels/${cid}/messages?limit=50`, {
        ...
        for (const m of msgs) {
          ...
          if (hit && (!best || BigInt(m.id) > BigInt(best.msg.id))) {
            best = { msg: m, channelId: cid, link: hit };
          }
        }
      }
      if (!best) return json({ ok: false, reason: "No axiom.trade link found in any enabled channel" });
  ...
  try {
    ...
    for (const { listenChannelId, replyChannelId, section: ownerSection, cfg } of uniqueChannels) {
      ...
      for (const msg of messages) {
        ...
        // ═══ RAID COMMUNITY AUTO-POST (DISABLED) ═══
        // Auto-posting to X community removed — only Home Comm scheduled campaigns are active now.
