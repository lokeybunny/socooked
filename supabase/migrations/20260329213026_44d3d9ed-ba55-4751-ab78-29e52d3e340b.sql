
CREATE OR REPLACE FUNCTION public.auto_send_lead_report()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  _url text;
  _anon_key text;
  _response extensions.http_response;
  _payload jsonb;
BEGIN
  _payload := jsonb_build_object(
    'lead_id', NEW.id,
    'auto', true
  );

  _url := 'https://mziuxsfxevjnmdwnrqjs.supabase.co/functions/v1/lead-report-email';
  _anon_key := 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im16aXV4c2Z4ZXZqbm1kd25ycWpzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzExNjgzMzQsImV4cCI6MjA4Njc0NDMzNH0.APi_x5YBKa8bOKpjLGiJUBB5qxi3rKKxWiApQAlf78c';

  SELECT * INTO _response FROM extensions.http((
    'POST',
    _url,
    ARRAY[
      extensions.http_header('apikey', _anon_key),
      extensions.http_header('Authorization', 'Bearer ' || _anon_key),
      extensions.http_header('Content-Type', 'application/json')
    ],
    'application/json',
    _payload::text
  )::extensions.http_request);

  RETURN NEW;
END;
$function$;

DROP TRIGGER IF EXISTS trg_auto_send_lead_report ON public.lw_landing_leads;

CREATE TRIGGER trg_auto_send_lead_report
  AFTER INSERT ON public.lw_landing_leads
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_send_lead_report();
