...
BEGIN
  ...
  -- Skip noisy automated communication syncs to avoid notification spam
  IF TG_ARGV[0] = 'communication' AND TG_OP != 'DELETE' THEN
    _provider := _row->>'provider';
    IF _provider IN (
      'gmail-read-tracker',
      'upload-post-dm-notify',
      'gmail',
      'gmail-poll'
    ) THEN
